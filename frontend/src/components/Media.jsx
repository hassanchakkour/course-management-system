...
const Media = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      console.log("asdasdads", title, description, file);
      const formData = {
        title: title,
  ...
  const remainingCharacters = formik.values.description
    .replace(/(<([^>]+)>)/gi, "")
    .trim().length;
  const rest = 3000 - remainingCharacters;

  // console.log("asdasdadsasadsad", file);
  return (
    <Container
      maxWidth="100%"
      sx={{
        ...
        <Box marginBottom="30px" color="gray">
          <div style={{ display: "flex" }}>
            ...
            >
              ...
              <h1 class="text-1xl text-white font-bold m-0">Media</h1>
            </div>
          </div>
        </Box>
        ...
        <Box marginBottom="20px">
          <div className="flex items-center">
            ...
            <div style={{ width: "100%", marginTop: "0px" }}>
              <input
                className={`w-full bg-gray-800 text-gray  border text-sm border-white rounded-lg px-3 py-2 ${
